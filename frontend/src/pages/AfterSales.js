...
const TYPES = ['All', 'RMA', 'Warranty Claim', 'EOL Notice'];
const WARRANTY = ['All', 'In Warranty', 'Out of Warranty', 'N/A'];
const PAGE_SIZE = 5;

const typeStyle = {
    'RMA': 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400',
    'Warranty Claim': 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
    'EOL Notice': 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400',
};
const repairVariant = s => {
    if (s === 'Repaired' || s === 'Replacement Shipped') return 'success';
    if (s === 'Under Diagnosis' || s === 'Notice Issued') return 'warning';
    return 'danger';
};
const slaBreach = r => r.turnaround_days != null && r.sla_days != null && r.turnaround_days > r.sla_days;
...
const AfterSales = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const data = await dataService.getAfterSales();
                setRecords(data);
            } catch (err) {
    ...
    const filtered = records.filter(r =>
        ...
        (warranty === 'All' || (r.warranty_status || r.warranty) === warranty) &&
        ((r.customer || '').toLowerCase().includes(search.toLowerCase()) ||
            (r.product || '').toLowerCase().includes(search.toLowerCase()) ||
            (r.rma_number && r.rma_number.toLowerCase().includes(search.toLowerCase())))
    );
        ...
        return (
            <PageLayout>
                <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-3 tracking-tight">Support System Fault</h2>
    ...
    return (
        <PageLayout>
            <PageHeader
                ...
                subtitle="RMA tracking, warranty intelligence, and product lifecycle support."
            />
            ...
            <PageContent>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                        ['Open RMAs', records.filter(r => r.type === 'RMA' && !(r.status || '').startsWith('Repaired') && !(r.status || '').startsWith('Closed')).length, 'text-blue-600', 'bg-blue-50 dark:bg-blue-900/20'],
                        ['Warranty Claims', records.filter(r => r.type === 'Warranty Claim').length, 'text-amber-600', 'bg-amber-50 dark:bg-amber-900/20'],
                        ['SLA Breaches', records.filter(slaBreach).length, 'text-rose-600', 'bg-rose-50 dark:bg-rose-900/20'],
                        ['EOL Notices', records.filter(r => r.type === 'EOL Notice').length, 'text-orange-600', 'bg-orange-50 dark:bg-orange-900/20'],
                    ].map(([label, val, tc, bg]) => (
                        <Card key={label}>
                ...
                <PageActions>
                    <div className="relative flex-1 max-w-sm">
                        ...
                        <input placeholder="Search RMA, client or item…" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }}
                    ...
                    {[['Type', TYPES, type, setType], ['Warranty', WARRANTY, warranty, setWarranty]].map(([label, opts, val, set]) => (
                    ...
                    <div className="responsive-table-container custom-scrollbar">
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {paged.length === 0 ? (
                                    <tr><td colSpan={8} className="px-5 py-12 text-center text-sm text-slate-400">No records match your filters.</td></tr>
                                ) : paged.map(r => (
                                        ...
                                        <td className="px-5 py-4 text-center">
                                            {r.tat_compliance ? (
                                                <span className={`text-xs font-bold tracking-tight px-2 py-1 rounded-lg ${
                                                    r.tat_compliance === 'Breached' ? 'bg-rose-50 text-rose-600 dark:bg-rose-900/20' : 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20'
                                                }`}>{r.tat_compliance}</span>
                                            ) : r.turnaround_days != null ? (
                                                <span className={`text-xs font-bold tracking-tight tabular-nums px-2 py-1 rounded-lg ${slaBreach(r) ? 'bg-rose-50 text-rose-600 dark:bg-rose-900/20' : 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20'}`}>{r.turnaround_days}d / {r.sla_days}d</span>
                                            ) : (
                                                <span className="text-slate-400">—</span>
                                            )}
                                        </td>
                                        <td className="px-5 py-4 text-center"><Badge variant={repairVariant(r.status)}>{r.status}</Badge></td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
